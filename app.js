...
function initializeEditor() {
    ...
    setTimeout(() => {
        if (!fileManager.currentFile) {
            const defaultFile = fileManager.createFile(null, 'Main.java');
            if (defaultFile) {
                fileManager.setCurrentFile(defaultFile.id);
                document.getElementById('currentFileName').textContent = 'Main.java';
                document.getElementById('currentFileNameMobile').textContent = 'Main.java';
                localStorage.setItem('lastOpenedFileId', defaultFile.id);
                renderProjects();
            }
        }
        saveCurrentFile();
...
function executeWithAPI(code, inputs) {
    ...
    const BACKEND_URL = window.BACKEND_URL || 'https://java-editor.onrender.com';
